...
describe("register a user", () => {
  ...
  it("should return status 500 on server fail", async () => {
    ...
    const customError = new Error("internal Error");
    jest.spyOn(UserService, "register").mockRejectedValueOnce(customError);
  ...
  it("should return status 400 if user is already registered", async () => {
    ...
    const customError = new Error("duplication error");

    jest.spyOn(UserService, "register").mockRejectedValue(customError);

    await UserController.register(req, res);

    expect(res.status).toBeCalledWith(400);
    expect(res.sendStatus).not.toHaveBeenCalled();
    expect(res.send).toHaveBeenCalledWith("user already exists in the system");
  });
});
...
describe("login", () => {
  ...
  it("should response 401 if no such user ", async () => {
    jest
      ...
      .mockRejectedValueOnce(new Error("Not exist error"));
